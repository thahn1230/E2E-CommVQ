...
class Trainer:
    ...
    def _inner_training_loop(
        ...
        tr_kv_vq_loss = torch.tensor(0.0).to(args.device)
        tr_ce_loss = torch.tensor(0.0).to(args.device)
        tr_kv_mse_loss = torch.tensor(0.0).to(args.device)
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                with self.accelerator.accumulate(model):
                    tr_loss_step, other_loss_step = self.training_step(model, inputs)
                ...
                kv_vq_loss = other_loss_step.get("kv_vq_loss", torch.tensor(0.0).to(tr_loss.device))
                ce_loss = other_loss_step.get("ce_loss", torch.tensor(0.0).to(tr_loss.device))
                kv_mse_loss = other_loss_step.get("kv_mse_loss", torch.tensor(0.0).to(tr_loss.device))
                mse_loss_array = other_loss_step.get("mse_loss_array", None)
                vq_loss_array = other_loss_step.get("vq_loss_array", None)
                tr_kv_vq_loss += kv_vq_loss
                tr_ce_loss += ce_loss
                tr_kv_mse_loss += kv_mse_loss
                ...
                if (
                    ...
                    loss_dict = {
                        "loss": tr_loss,
                        "kv_vq_loss": tr_kv_vq_loss,
                        "ce_loss": tr_ce_loss,
                        "kv_mse_loss": tr_kv_mse_loss,
                        "mse_loss_array": mse_loss_array,
                        "vq_loss_array": vq_loss_array,
                    }
                    self._maybe_log_save_evaluate(loss_dict, grad_norm, model, trial, epoch, ignore_keys_for_eval)
                else:
    ...
    def _maybe_log_save_evaluate(self, loss_dict, grad_norm, model, trial, epoch, ignore_keys_for_eval):
        if self.control.should_log and self.state.global_step > self._globalstep_last_logged:
            ...
            tr_loss = loss_dict["loss"]
            tr_kv_vq_loss = loss_dict["kv_vq_loss"]
            tr_ce_loss = loss_dict["ce_loss"]
            tr_kv_mse_loss = loss_dict["kv_mse_loss"]
            mse_loss_array = loss_dict.get("mse_loss_array", None)
            vq_loss_array = loss_dict.get("vq_loss_array", None)
            # all_gather + mean() to get average loss over all processes
            tr_loss_scalar = self._nested_gather(tr_loss).mean().item()
            tr_kv_vq_loss_scalar = self._nested_gather(tr_kv_vq_loss).mean().item()
            tr_ce_loss_scalar = self._nested_gather(tr_ce_loss).mean().item()
            tr_kv_mse_loss_scalar = self._nested_gather(tr_kv_mse_loss).mean().item()

            # reset tr_loss to zero
            tr_loss -= tr_loss
            tr_kv_vq_loss -= tr_kv_vq_loss
            tr_ce_loss -= tr_ce_loss
            tr_kv_mse_loss -= tr_kv_mse_loss

            logs["loss"] = round(tr_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 6)
            logs["kv_vq_loss"] = round(tr_kv_vq_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 6)
            logs["ce_loss"] = round(tr_ce_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 6)
            logs["kv_mse_loss"] = round(tr_kv_mse_loss_scalar / (self.state.global_step - self._globalstep_last_logged), 6)
            if mse_loss_array is not None:
                for III, x in enumerate(mse_loss_array):
                    if III % 2 == 0:
                        logs[f"key_mse_loss_layer_{str(III // 2).zfill(3)}"] = round(x, 6)
                    else:
                        logs[f"value_mse_loss_layer_{str(III // 2).zfill(3)}"] = round(x, 6)
            if vq_loss_array is not None:
                for III, x in enumerate(vq_loss_array):
                    logs[f"vq_loss_layer_{str(III).zfill(3)}"] = round(x, 6)
    ...
    def training_step(self, model: nn.Module, inputs: Dict[str, Union[torch.Tensor, Any]]) -> torch.Tensor:
        ...
        for k, v in outputs.items():
            if "loss" in k and k != "loss":
                if "array" in k:
                    other_loss[k] = [x.detach().item() / self.args.gradient_accumulation_steps for x in v]
                else:
                    other_loss[k] = v.detach() / self.args.gradient_accumulation_steps

        return (loss.detach() / self.args.gradient_accumulation_steps), other_loss
